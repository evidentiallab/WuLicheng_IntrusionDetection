...
def probabilistic_CNN_sequential(num_class,is_train, is_load, model_filepath,x_train,
                      y_train, x_test, y_test,pic_fp0,pic_fp1,output_confusion_matrix):
    model = Sequential()
...
def probabilistic_CNN_api(data_WIDTH,num_class,is_train, is_load, model_filepath,x_train,
                        y_train, x_test, y_test,pic_fp0,pic_fp1,output_confusion_matrix):
    ...
    if is_train == 1:
        h = model_PR.fit(x_train, y_train, batch_size=32, epochs=150,
                         verbose=1, callbacks=[checkpoint_callback], validation_data=(x_test, y_test), shuffle=True)
...
def evidential_CNN_api(data_WIDTH, num_class, prototypes,nu,model_filepath,evi_filepath,mid_filepath,
                       load_and_train, is_load,plot_DS_layer_loss,
    ...
    dr1 = keras.layers.Dropout(0.5)(bn1)
    flatten1 = tf.keras.layers.Flatten()(dr1)
    # dense1 = tf.keras.layers.Dense(num_class, activation='softmax')(flatten1)
...
def set_value_api(data_WIDTH,num_class, number_act_set, act_set, nu, tol,prototypes,
    ...
    if is_load:
        model_evi_SV.layers[-1].set_weights(tf.reshape(utility_matrix, [1, 1023, 10]))
