...
def probabilistic_biSNN(data_WIDTH,num_class,is_train, is_load, model_filepath,x_train,
                        y_train, x_test, y_test,output_confusion_matrix):
    inputs = keras.layers.Input((data_WIDTH, 1))
    c1 = keras.layers.Conv1D(32, 3, activation='selu', kernel_initializer='lecun_normal', padding='same')(inputs)
    c2 = keras.layers.Conv1D(32, 3, activation='selu', kernel_initializer='lecun_normal', padding='same')(c1)
    ...
    fc1 = tf.keras.layers.Dense(64,activation='selu', kernel_initializer='lecun_normal')(flatten1)
    outputs = tf.keras.layers.Dense(num_class, activation='softmax')(fc1)

    model_PR = tf.keras.Model(inputs=[inputs], outputs=[outputs])
    model_PR.compile(optimizer=keras.optimizers.RMSprop(learning_rate=0.0001, decay=1e-6), loss='CategoricalCrossentropy',
                     metrics=['accuracy'])
    # model_PR.summary()
...
def evidential_SNN(data_WIDTH, num_class, prototypes,nu,model_filepath,evi_filepath,mid_filepath,
