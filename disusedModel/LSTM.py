...
def probabilistic_LSTM(hidden_units,num_class,model_filepath,lr,
    ...
    # disusedModel.compile(optimizer=keras.optimizers.Nadam(learning_rate=lr, beta_1=0.9, beta_2=0.999, epsilon=None,
...
def bilayer_LSTM(hidden_units,num_class,model_filepath,lr,
    ...
    # disusedModel.compile(optimizer=keras.optimizers.SGD(learning_rate=lr, momentum=0.0, nesterov=False),
    #               loss='CategoricalCrossentropy',metrics=['accuracy'])
...
def evidential_LSTM_sequential(hidden_units, num_class, nu, is_train, prototypes, evi_filepath,
    ...
    model.add(Flatten())
    model.add(Dense(units=50))
    # disusedModel.add(Dense(units=num_class, activation='softmax'))
    ...
    # disusedModel.compile(optimizer=keras.optimizers.Nadam(learning_rate=0.001, beta_1=0.9, beta_2=0.999, epsilon=None, schedule_decay=0.004),loss='CategoricalCrossentropy',metrics=['accuracy'])
    model.compile(optimizer='adam', loss='CategoricalCrossentropy',metrics=['accuracy'])
    # disusedModel.summary()
