...
def utility_mtx(num_cls, actset, class_set):
...
def set_valued_evidential_FitNet4(num_class,number_act_set,act_set,
                                  prototypes,utility_matrix,is_load,
                                  x_train, y_train,
                                  x_test, y_test):
    data_WIDTH = 10
    data_HEIGHT = 10
    ...
    if is_load:
        ...
        resutls = tf.argmax(model_evi_SV.predict(x_test), -1)
        imprecise_results = []
        ...
        average_utility_imprecision = AU_imprecision.average_utility(utility_matrix, resutls, y_test, act_set)
